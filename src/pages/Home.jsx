...
const Home = () => {
  ...
  const [allProductsclone, setAllProductsclone] = useState(null);
  ...
  useEffect(() => {
    setHotPizza(allProducts ? allProducts.filter((item) => item.category === "Pizza").slice(0, 4):[]);
  }, []);

  useEffect(() => {
    if (category === "ALL") {
      setAllProducts(allProductsclone && allProductsclone);
    }

    if (category === "BURGER") {
      const filteredProducts = allProductsclone && allProductsclone.filter(
